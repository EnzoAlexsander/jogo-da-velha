...
let playerTime = 0; // jogadores
let symbols = ['o', 'x']; //símbolos
let gameOver = false; //o Jogo não acabou

let winStates = [
    // Variável dos estados que dão a vitória.
    [0, 1, 2],
    [3, 4, 5],
    [6, 7, 8],
...
function handleMove(position) {
    //Coloca um símbulo dentro do board.
    if (gameOver) {
    ...
    if (board[position] == '') {
        //verifica se a posição está vazia no tabuleiro.
        ...
        if (gameOver == false) {
            // Só passa para o próximo jogador se o gameOver for falso.
...
function isWin() {
    // Função para saber se temos um vencedor.
    ...
    for (let i = 0; i < winStates.length; i++) {
        ...
        if (
            board[pos1] == board[pos2] && // Verifica se os elementos são iguais.
